function [gammat,zref,pref,sigref] = gsw_gammat_analytic_CT(sr,ct)
%
...
rho = @(x2,xs,ys,z) 1./(v000 + xs.*(v100 + xs.*(v200 + xs.*(v300 + xs.*(v400 + xs.*(v500 ...
...
for n=1:30
    %disp(n)
    % Compute buoyancy 
    buoyancy = rhor(zref) - rho(x2,xs,ys,pr(zref)*1.e-4); 
...
pref = pr(zref); 
x = (pref-pmean)./pstd;
sigref = rho(x2,xs,ys,pref*1e-4) - 1000.; 
gammat = sigref - f(x); 
